...
inline void GLConsole::_RenderText()
{
    ...
    glEnable( GL_SCISSOR_TEST ); {
        ...
        TextWrite(m_nConsoleLeftMargin, lineLoc - m_nScrollPixels,
                "> " + m_sCurrentCommandBeg );
...
{ 
    ScrollUp( (int)((m_Viewport.height*m_fOverlayPercent) - 2*TextCharHeight())); 
}

////////////////////////////////////////////////////////////////////////////////
inline void GLConsole::ScrollDownPage()
{ 
    ScrollDown( (int)( (m_Viewport.height*m_fOverlayPercent) - 2*TextCharHeight() ) ); 
}
    ...
    if( !node ) {
        ...
        if( suggest.size() > 1) {
            ...
            for( unsigned int ii = 0; ii < suggest_name_index_set.size() ; ii++ ) {
                ...
                if( (int)(commands+tmp).length() > m_Viewport.width/TextCharWidth() ) {
            ...
            for( unsigned int ii = 0; ii < suggest_name_index_set.size() ; ii++ ) {
                std::string tmp = suggest_name_index_set[ii].first;
                tmp.resize( longest, ' ' );
                if( (int)(functions+tmp).length() > m_Viewport.width/TextCharWidth() ) {
